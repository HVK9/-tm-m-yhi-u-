--[[
    HVK9 ULTIMATE V14 - STABLE FLIGHT & NOCLIP EDITION
    1. Fast Attack: Giữ nguyên cơ chế RegisterAttack/RegisterHit.
    2. Auto Farm: Tối ưu hóa bay (hết rung lắc) + Tự động Noclip (xuyên tường).
    3. Data: Giữ nguyên 100% format cũ.
]]
...
local CONFIG = {
    ...
    FlySpeed = 300,
    HoverHeight = 15, -- Khoảng cách treo trên đầu quái
}

-- --- DATA TỌA ĐỘ (GIỮ NGUYÊN 100% CỦA BẠN) ---
local MOBS_DB = {
    -- ================= SEA 1 =================
    -- Starter Island (0 - 10)
    ...
    {Name = "Yeti", Level = 105, Pos = Vector3.new(1222, 138, -1489)}, -- Boss
    -- Marine Fortress (120 - 150)
    {Name = "Chief Petty Officer", Level = 120, Pos = Vector3.new(-4996, 84, 4154)},
    {Name = "Vice Admiral", Level = 130, Pos = Vector3.new(-5080, 154, 4428)}, -- Boss
    ...
    {Name = "Warden", Level = 220, Pos = Vector3.new(5284, 94, 922)}, -- Boss
    {Name = "Chief Warden", Level = 230, Pos = Vector3.new(5284, 94, 922)}, -- Boss
    {Name = "Swan", Level = 240, Pos = Vector3.new(5284, 94, 922)}, -- Boss
    -- Colosseum (225 - 300)
    ...
    {Name = "Fishman Lord", Level = 425, Pos = Vector3.new(-5685, 17, 8648)}, -- Boss
    ...
    {Name = "Island Empress", Level = 1675, Pos = Vector3.new(-1194, 103, 6916)}, -- Boss
    -- Great Tree (1700 - 1750)
    {Name = "Marine Commodore", Level = 1700, Pos = Vector3.new(2450, 70, -7350)},
    {Name = "Kilo Admiral", Level = 1750, Pos = Vector3.new(2904, 509, -7368)}, -- Boss
...
-- 2. TỰ ĐỘNG NOCLIP (CHẠY LIÊN TỤC KHI FARM)
-- Giúp bay xuyên tường, không bị kẹt
...
-- 5. Main Loop Auto Farm (Cải tiến ổn định vị trí)
RunService.RenderStepped:Connect(function()
    if CONFIG.AutoFarm and CONFIG.TargetPosition then
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if targetMob then
                -- Bay tới đầu quái (TƯ THẾ ỔN ĐỊNH)
                local farmPos = targetMob.HumanoidRootPart.Position + Vector3.new(0, CONFIG.HoverHeight, 0)
                
                -- CẬP NHẬT MỚI: Chỉ set vị trí, không set hướng nhìn vào quái để tránh rung lắc
                hrp.CFrame = CFrame.new(farmPos) 
                
                -- Giữ vị trí bằng BodyVelocity
                ...
                -- GIỮ THĂNG BẰNG bằng BodyGyro (Chống lật, chống rung màn hình)
                if not hrp:FindFirstChild("BodyGyro") then
                    local bg = Instance.new("BodyGyro", hrp)
                    bg.P = 3000
                    bg.D = 100
                    bg.MaxTorque = Vector3.new(9e9, 9e9, 9e9)
                    bg.CFrame = hrp.CFrame -- Khóa góc hiện tại
                else
                    -- Cập nhật Gyro giữ nhân vật thẳng đứng hoặc theo ý muốn
                    hrp.BodyGyro.CFrame = CFrame.new(farmPos)
                end
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if char.HumanoidRootPart:FindFirstChild("StableGyro") then char.HumanoidRootPart.StableGyro:Destroy() end
            if char.HumanoidRootPart:FindFirstChild("BodyGyro") then char.HumanoidRootPart.BodyGyro:Destroy() end
        end
    end
end)

StarterGui:SetCore("SendNotification", {Title = "HVK-9"; Text = "cần hỗ trợ liên hệ 0896.XXX.XXX"; Duration = 5})
